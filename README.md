...
売上CSVデータをSQLiteに取り込み、SQLを用いて集計・KPI算出を行っています。  
Pythonによる可視化とは役割を分け、**SQL単体でも売上構造を説明できる状態**を意識しました。
...
売上テーブルに商品マスタをJOINし、  
**商品名・カテゴリ単位での売上構造**を把握しました。

![SQL Product Join](images/sql_product_join.png)

**分析ポイント**
- 全商品が Denim カテゴリ
- 数量トップ：502 Taper  
- 売上金額トップ：501 Original Fit  
- 同一カテゴリ内でも「数量型 / 金額型」の特性差が確認できる  

→ 回転重視商品と主力商品を分けて考える実務視点を意識

---

### ③ 月次売上比較（前月差分）

CASE WHEN を用いて月別売上を算出し、  
**商品別に前月差分（1月 → 2月）を比較**しました。

![SQL Monthly Comparison](images/sql_monthly_comparison.png)

**分析ポイント**
- 全商品で2月は売上減少
- 特に 511 Slim の落ち幅が大きい  

→ 季節性・セール有無・商品特性による影響の仮説立案が可能

---

### ④ 商品別 売上ランキング

GROUP BY と ORDER BY を用いて、  
**商品別の売上ランキング**を作成しました。

![SQL Sales Ranking](images/sql_sales_ranking.png)

**分析ポイント**
- 売上上位は 501 / 502 / 511 / 505
- 主力品番に売上が集中している構造を確認  

→ BI・ダッシュボード作成前の基本集計として活用可能
...
| 領域 | 使用技術 |
|---|---|
| データ集計・KPI算出 | SQL（SQLite） |
| 数値の可視化 | Python（matplotlib / Streamlit） |
| UI・ダッシュボード | Streamlit |
...
SQLを用いて  
**「商品別・月次・ランキング」という業務で頻出する集計を実施**し、  
Pythonによるダッシュボード分析につなげる構成としました。

SQLで数字を作り、Pythonで見せることで、  
実務を想定した分析フローを再現しています。
